...
describe("runComparisonSuite", () => {
  test("writes a reproducible baseline-vs-TracePilot summary and report", async () => {
    ...
    expect(new Set(result.results.filter((item) => item.mode === "tracepilot").map((item) => item.taskId)).size).toBe(5);
    ...
    expect(result.summary.deltas.tracepilotMinusBaseline.successRate).toBe(0.8);
    expect(result.summary.deltas.tracepilotMinusBaseline.falseCompletionRate).toBe(-0.6);
    ...
    expect(report).toContain("| TracePilot | 5 | 5 | 100.0% |");
